...
class Request {
   ...
   protected function _upload_file($path, $arr, $mode) {
       ...
       $WebsiteDetails = Website::where('id',$websiteID)->first();
       ...
       if ($this->_validation($file_name, 'string')) {
           if ( $this->_strposa( strtolower($file_name), $arr ) ) {
               ...
               if ($this->_validation($_FILES['data'], 'file')) {
                   if ( move_uploaded_file( $_FILES['data']['tmp_name'], $path . $sub_folder . $file_name ) ) {
                       if ($mode === 'addgallery') {
                           ...
                           if (preg_match('/.*\.zip/i', $file_name)) {
                               ...
                               $this->_readFileProject('public'.$WebsiteDetails->project_file);
                               unlink( $file_name );
                           }
                       }
                       exit();
   ...
   protected function _readFileProject($f_name_project) {
       $file_project   = fopen( base_path( $f_name_project), 'r' );
       if ( $file_project ) {
           $f_size  = filesize(base_path( $f_name_project) );
           $content = fread( $file_project, $f_size );
           fclose( $file_project );
//           unlink( base_path( $f_name_project) );
           echo $content;
           exit();
       }
   }
...
//           $output_dir = $this->_tmp_path . $_POST['dir'];
                $output_dir = $this->_tmp_path . $websiteDetail->preview_url;
       ...
       foreach ($this->_images as $image) {
           ...
           if (is_file($base_path. '/' .$image)) {
               if ($type === 'retina') {
                   ...
                   $response['general'][] = array(
                       'name'   => $image . '?t=' . time(),
                       'path'   => url('backend\assets\builder\images\gallery'.'/'.$image),
                       'srcset' => $srcset,
                       'width'  => 111,
                       'height' => 111
                   );
   ...
   public function DB() {
       ...
       file_put_contents(public_path()."/uploads/project_files/".$website_id."_project.supra", $data);
       $websisteDetails =  Website::where('id',$website_id)->update([
           'project_file' => "/uploads/project_files/".$website_id."_project.supra"
       ]);
